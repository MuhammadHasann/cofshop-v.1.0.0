      ...
      <div className="relative flex justify-center items-center w-full h-36 bg-gray-200 rounded md:h-40 lg:h-48 xl:h-72">
        ...
        <button className="absolute top-1 right-1">
          <svg
            ...
            strokeWidth={1.5}
            stroke="currentColor"
            className="w-5 h-5 fill-transparent transition-all duration-300 ease-in-out hover:fill-red-200 hover:stroke-red-500 lg:w-6 lg:h-6 xl:w-7 xl:h-7"
          >
            <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
          </svg>
        </button>
      </div>
      <h1 className="mt-2 mb-1 w-full h-8 text-xxs font-semibold lg:mt-3 lg:h-12 lg:text-xs xl:h-14 xl:text-sm">{name}</h1>
      <div className="flex justify-between items-center">
        <h2 className="text-sm font-bold lg:text-base xl:text-lg">${price}</h2>
        <div className="flex items-center">
          ...
          <p className="text-xs lg:text-sm">{rate}k</p>
        </div>
      </div>
    </div>
  );
